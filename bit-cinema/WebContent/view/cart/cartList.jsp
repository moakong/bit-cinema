	...
	<div class="container">
		...
		<div class="row">
			<div>
				<table>
					<tr>
						<th colspan="5"><input type="checkbox" id="all" name="all" value="all"
							onclick="changeChk()" /> <label for="all" onclick="changeChk()">전체선택</label><br>
						</th>
					</tr>
					<tr class="a">
						<th id="ck">선택</th>
					...
					<form action="DeleteCart" method="post" name="cart">
						<c:forEach var="cart" items="${list}" varStatus="s">
							<tr class="list">
								...
								<td><a	href="${pageContext.request.contextPath}/snack/DetailSnack?snackId=<c:out value="${cart.snackId}"/>">
								...
								<c:out value="${cart.name}" />
								 ...
								 <label for="${s.index}">
										<td id="tt"><c:out value="${cart.price}" />원</td>
										 ...
										 <td id="tt">총&nbsp;<c:out value="${cart.price * cart.count}"/>원</td>
								</label>
							</tr>
							<br>
						</c:forEach>
						<tr>
							<td colspan="4"><input type="submit" onclick="return confirm('삭제하시겠습니까?')" value="삭제" /></td>
